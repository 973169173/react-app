...
def get_analysis_results_api():
    """
    获取某 FuncObject 某表的分析结果列表（图表元数据+数据）。
    Query 参数：
      - fo_name: FuncObject 名称（必填）
      - table_name: 表名（必填）
    返回：{ results: [ { chart_id, chart_type, title, x_label, y_label, ... } ], count }
    注意：x_data/y_data/labels/values 在后端侧以字符串形式存储，前端可视需要时自行解析。
    """
    if _op_impl is None:
        return jsonify({'error': 'backend not ready: OperationImplementation init failed'}), 500
    try:
