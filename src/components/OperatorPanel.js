          ...
          <div className="operator-grid">
            ...
            {operator.output && (
              <div className="output-section">
                {/* 保留原有内联展示，同时提供“View Result”按钮打开更强的 ResultViewer 弹窗 */}
                <Space style={{ marginBottom: 8 }}>
                  <Button size="small" type="primary" onClick={() => onOpenResult && onOpenResult(operator)}>
                    View Result
                  </Button>
                </Space>
                <Collapse size="small">
                        ...
                        if (Array.isArray(data?.columns) && Array.isArray(data?.data)) {
                          ...
                          return (
                            <Table
                              ...
                              onRow={(record) => ({
                                onClick: (event) => {
                                  // 获取点击的列：从点击的 td 定位列索引，再映射到可见列的 key
